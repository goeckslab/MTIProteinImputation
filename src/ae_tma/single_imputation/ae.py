...
logging.basicConfig(level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s',
                    handlers=[
                        logging.FileHandler(Path("src", "ae", "", "debug.log")),
...
def create_results_folder(spatial_radius: str) -> [Path, int]:
    save_folder = Path(f"src/ae_tma/single_imputation", patient_type)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-m", "--mode", required=True, choices=["exp"], default="ip")
    ...
    parser.add_argument("-rm", "--replace_mode", action="store", type=str, choices=["mean"], default="zero")
    ...
    if patient_type == "exp":
        ...
        train_data = pd.read_csv(Path("data", "tma", "tma_cleaned.tsv"), sep="\t")
        train_data = train_data[train_data["Core"] != test_biopsy_name]
        ...
        # Load test data
        test_data = pd.read_csv(Path("data", "tma", "tma_cleaned.tsv"), sep="\t")
        test_data = test_data[test_data["Core"] == test_biopsy_name]
    ...
    for i in tqdm(range(1, subsets)):
        ...
        # Predict
        impute_markers(scores=scores, all_predictions=predictions, mode=patient_type, spatial_radius=int(0),
