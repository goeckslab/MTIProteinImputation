...
# Function to find tiles with matching coordinates across markers
def find_matching_tiles_old(df):
    # Group by coordinates
    grouped = df.groupby(['x_start', 'x_end', 'y_start', 'y_end'])

    # Filter groups that have all markers
    matching_groups = []
    for name, group in grouped:
        if len(group['Marker'].unique()) == len(df['Marker'].unique()):
