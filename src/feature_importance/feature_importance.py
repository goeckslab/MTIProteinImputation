...
save_path: Path = Path("results", "feature_importance")
...
if __name__ == '__main__':
    ...
    for target_protein in ["CD45", "CK19"]:
        ...
        assert X_train.shape[1] == X_test.shape[1], "Train and test data shape is not similar"
        assert X_train.shape[1] == train_data_proteins.shape[1], "Train data shape is not correct"
        assert X_test.shape[1] == test_data_proteins.shape[1], "Test data shape is not correct"

        # Generating and training on bootstrap samples
        for i in range(1):
            og_model, original_score = classifier(X_train, train_data_treatment, X_test, test_data_treatment)
            explainer = shap.Explainer(og_model)

            X_test = pd.DataFrame(X_test, columns=SHARED_MARKERS)
            ...
            plt.title(f"Protein {target_protein} Original data")
            plt.savefig(Path(save_path, f"{target_protein}_og_feature_importance.png"))
            plt.close(fig)

            fig = plt.figure()
            shap.plots.bar(shap_values[:, :, 1], show=False)
            plt.title(f"Protein {target_protein} Original data")
            plt.savefig(Path(save_path, f"{target_protein}_og_feature_importance_bar.png"))
            plt.close(fig)
            ...
            im_model, imputed_score = classifier(X_train_imp, train_data_treatment, X_test_imp, test_data_treatment)
            explainer = shap.Explainer(im_model)
            X_test = pd.DataFrame(X_test, columns=SHARED_MARKERS)
            shap_values = explainer(X_test)
            ...
            fig = plt.figure()
            shap.plots.beeswarm(shap_values[:, :, 1], show=False)
            plt.title(f"Protein {target_protein} Imputed data")
            plt.savefig(Path(save_path, f"{target_protein}_im_feature_importance.png"))
            plt.close(fig)

            fig = plt.figure()
            shap.plots.bar(shap_values[:, :, 1], show=False)
            plt.title(f"Protein {target_protein} Imputed data")
            plt.savefig(Path(save_path, f"{target_protein}_im_feature_importance_bar.png"))
            plt.close(fig)
            ...
            rem_model, removed_score = classifier(X_train_removed, train_data_treatment, X_test_removed,
                                                  test_data_treatment)

            explainer = shap.Explainer(rem_model)
            sub_list = [SHARED_MARKERS[i] for i in range(len(SHARED_MARKERS)) if
                        i != SHARED_MARKERS.index(target_protein)]
            X_test_removed = pd.DataFrame(X_test_removed, columns=sub_list)
            shap_values = explainer(X_test_removed)

            fig = plt.figure()
            # summarize the effects of all the features
            shap.plots.beeswarm(shap_values[:, :, 1], show=False)
            plt.title(f"Protein {target_protein} Removed data")
            plt.savefig(Path(save_path, f"{target_protein}_removed_feature_importance.png"))
            plt.close(fig)

            fig = plt.figure()
            shap.plots.bar(shap_values[:, :, 1], show=False)
            plt.title(f"Protein {target_protein} Removed data")
            plt.savefig(Path(save_path, f"{target_protein}_removed_feature_importance_bar.png"))
            plt.close(fig)
