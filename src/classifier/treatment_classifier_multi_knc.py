...
save_path: Path = Path("results", "classifier_multi", "knc")
...
if __name__ == '__main__':
    ...
    for round in rounds["Round"].unique():
        ...
        for i in range(30):
            og_model, _, og_probs = classifier(X_train, train_data_treatment, X_test, test_data_treatment)

            og_correct_cells, og_correct_treatment = get_correctly_confident_cells(og_model, og_probs, X_test,
                                                                                   test_data_treatment)
            ...
            # check og_correct_cells and og_correct_treatment has same shape
            assert og_correct_cells.shape[0] == og_correct_treatment.shape[
                0], "Correct cells and treatment shape is not similar"
            ...
            # replace target protein with imputed data using loc
            X_train_imp.loc[:, target_proteins] = train_imputed_data[target_proteins].values

            X_test_imp: pd.DataFrame = pd.DataFrame(X_test.copy(), columns=SHARED_MARKERS)
            # replace target protein with imputed data using loc
            X_test_imp.loc[:, target_proteins] = test_imputed_data[target_proteins].values
            ...
            assert X_train_imp.loc[:, remaining_markers].equals(
                X_train.loc[:, remaining_markers]), "Train data is not equal"

            assert X_test_imp.loc[:, remaining_markers].equals(
                X_test.loc[:, remaining_markers]), "Test data is not equal"
            ...
            imp_model, _, imp_probs = classifier(X_train_imp, train_data_treatment, X_test_imp,
                                                 test_data_treatment)
            ...
            assert not any([protein in X_train_removed.columns for protein in
                            target_proteins]), "Target protein is still in the data"
            assert not any([protein in X_test_removed.columns for protein in
                            target_proteins]), "Target protein is still in the data"
