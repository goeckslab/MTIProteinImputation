...
def create_results_folder(spatial_radius: str, test_biopsy_name: str, replace_value: str) -> [Path, int]:
    save_folder = Path(RESULTS_FOLDER, patient_type)
...
if __name__ == '__main__':
    ...
    rounds_per_marker = pd.read_csv(Path(ROUNDS_FOLDER))
    ...
    if patient_type == "ip":
        ...
        base_path = DATA_FOLDER if spatial_radius == "0" else f"{str(DATA_FOLDER)}_{spatial_radius}_µm"
